...
class DRN:
    def __init__(self):
        ...
        ScoreNetwork_Args,AutoDesign_Args = self._build_network()
        self.sess = tf.Session()
        #initialize_op = tf.variables_initializer(AutoDesign_Args)
        if os.path.isfile(os.path.join(sys.path[0], r'my_net/checkpoint')):
            self._load_data(ScoreNetwork_Args)
        #self.sess.run(initialize_op)
        print('NetWork init!')
        
    def _load_data(self,ScoreNetwork_Args):
        Saver = tf.train.Saver(ScoreNetwork_Args)
    ...
    def _build_network(self):
        ...
        # auto-design network input
        self.Background = tf.placeholder(tf.float32, [256,192,3], name='Background')
        self.ElementList = tf.placeholder(tf.float32, [None,256,192,4], name='ElementList')
        self.Tags = tf.placeholder(tf.float32,[None, 5], name='Tags')

        #--------------------------------- Design Feature Network ----------------------------
        def DesignFeatureNetwork(Input, ReUse = False):
            with tf.variable_scope('design_feature_network', reuse = ReUse):
                ...
                with tf.variable_scope('fc1') as variable_scope: 
                    ...
                    output = tf.nn.dropout(leaky_relu(tf.matmul(h_pool4_flat, w_fc1) + b_fc1), self.keep_prob)
        
            #-------------------------------------------- Unconv Unpool Network ---------------------------------------
            if not ReUse:
            ...
            return output
        ImgsFeature = DesignFeatureNetwork(self.Imgs, False)

        # ------------------------------------------- Semantic Scoring Network -----------------------------
        def SemanticScoringNetwork(Input, ReUse = False):
            with tf.variable_scope('semantic_scoring_network', reuse = ReUse):
                ...
                with tf.variable_scope('fc2') as variable_scope: 
                    ...
                    h_fc2 = tf.nn.dropout(leaky_relu(tf.matmul(Input, w_fc2) + b_fc2), self.keep_prob)
                ...
                with tf.variable_scope('fc3') as variable_scope: 
                    ...
                    op_fc3 = tf.nn.dropout(leaky_relu(tf.matmul(h_fc2, w_fc3) + b_fc3), self.keep_prob)
                
            with tf.variable_scope('Output', reuse = ReUse):
                w_op = tf.get_variable('w_op', regularizer=l2_reg, initializer=weight_variable([128, 5]))
                Score = tf.nn.softmax(tf.matmul(op_fc3, w_op))
            return Score
        self.Score = SemanticScoringNetwork(ImgsFeature)
        ...
        ScoreNetwork_Args = set(tf.all_variables())
        #--------------------------------- Re-Design Network ----------------------------
        ...
        def FN(Input, Shape, UseDropOut = False):
            w = tf.get_variable('w', regularizer=l2_reg, initializer=weight_variable(Shape))
            b = tf.get_variable('b', regularizer=l2_reg, initializer=bias_variable([Shape[1]], 0.5))
            if UseDropOut:
                return tf.nn.dropout(leaky_relu(tf.matmul(Input, w) + b), self.keep_prob)
            else:
                return leaky_relu(tf.matmul(Input, w) + b)
            
        def ElementNetwork(Input, Reuse = True):
            # input shape:[1,256,192,4], output shape: [1,256]
            Input = tf.reshape(Input, [-1,256,192,4])
            with tf.variable_scope('Element_feature_network', reuse = Reuse):
        ...
        def BackgroundNetwork(Input, Reuse = True):
            # input shape:[1,256,192,3], output shape: [1,256]
            Input = tf.reshape(Input, [-1,256,192,3])
            with tf.variable_scope('Background_feature_network', reuse = Reuse):
                #(NameScope, Shape, use_4x4_Pooling)
        ...
        def LabelNetwork(Input, Reuse = True):
            # input shape:[1,5], output shape: [1,64]
            Input = tf.reshape(Input, [-1,5])
            with tf.variable_scope('Label_segmantic_network', reuse = Reuse):
        ...
        def cond(index, backgroud, ElementList, DesignList, Tag):
            return index<tf.shape(ElementList)[0]
        
        def body(index, backgroud, ElementList, DesignList, Tag):
            LabelFeature = LabelNetwork(Tag)
            ElementFeature = ElementNetwork(ElementList[index])
            BackGroundFeature = BackgroundNetwork(backgroud)
            Design = DesignNetwork(LabelFeature, ElementFeature, BackGroundFeature)
            _backgroud = paste(backgroud, ElementList[index], Design[0])
            DesignList = DesignList + tf.pad(Design, [[index, tf.shape(self.ElementList)[0]-index-1], [0,0]], "CONSTANT")
            return index+1, _backgroud, ElementList, DesignList, Tag

        temp = set(tf.all_variables())
        '''
        LabelFeature = LabelNetwork(self.Tags,False)
        ElementFeature = ElementNetwork(self.ElementList,False)
        BackGroundFeature = BackgroundNetwork(self.Background, False)
        Design = DesignNetwork(LabelFeature, ElementFeature, BackGroundFeature, False)
        DesignList = tf.tile(tf.constant([[0.,0.,0.,0.]]), [tf.shape(self.ElementList)[0],1])
        index, self.DesignImg, ElementList, self.DesignList, Tag = tf.while_loop(cond, body, [0, self.Background, self.ElementList, DesignList, self.Tags])
        AutoDesign_Train_Args = set(tf.trainable_variables())

        #--------------------------------------   Auto Design   --------------------------
        ImgsFeature = DesignFeatureNetwork(self.DesignImg, True)
        ScoreOutput = SemanticScoringNetwork(ImgsFeature, True)
        with tf.variable_scope('Auto_Design'):
            self.AutoDesignLoss = tf.losses.mean_squared_error( self.Tags, ScoreOutput)
            self._train_auto_design = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.5).minimize(self.AutoDesignLoss, global_step=global_steps, var_list = list(AutoDesign_Train_Args))
        '''
        
        AutoDesign_Args = set(tf.trainable_variables()) - temp
        print(AutoDesign_Args)
        return ScoreNetwork_Args,AutoDesign_Args
    ...
    def AutoDesign(self, background, ElementList, Tag):
        DesignList, DesignImg, AutoDesignLoss = self.sess.run([self.DesignList, self.DesignImg, self.AutoDesignLoss],
                    feed_dict = {
        ...
        print(AutoDesignLoss)
        DesignImg /= 255
        plt.imshow(DesignImg)
        plt.show()
        print(DesignList, DesignImg.shape)
